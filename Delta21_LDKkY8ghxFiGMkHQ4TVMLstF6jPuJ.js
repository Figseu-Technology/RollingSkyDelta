...
let data;
let reqId;
$.getJSON('Delta21_LDKkY8ghxFiGMkHQ4TVMLstF6jPuJ.rse', d => {
  data = d;
  loadLevel(level);
  $('#play').show();
  $('#PresentCode').hide();
  ...
  $('#next').hide();
  $('#score').hide();
  $('#level-d').html('エンドコンテンツ');
  $('#level').html('Piknik Color');
  $('#stars').html('　');
  $('#Waves').html('Δ × ' + $.cookie('delta_21_wave'));
...
function start(e) {
  ...
  if (!started) {
    ...
    if (typeof wave == 'undefined') {
        wave = 0;
        $.cookie('delta_21_wave', wave, { expires: 40, domain:'figseu-technology.github.io'});
    }
    $('#zero').get(0).play();
    wave++;
    reset();
...
function gameover() {
  ...
  $.cookie('delta_21_wave', wave, { expires: 400, domain:'figseu-technology.github.io'});
  started = false;
  ball.speed.z = 0;
  $('#main').fadeIn(100);
  $('#retry').hide();
